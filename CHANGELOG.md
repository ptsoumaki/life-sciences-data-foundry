...
### Added
- **Nextflow Multi-Omics to OMOP Workflow (`pipelines/multi_omics_omop.nf`)**:
  - Implemented end-to-end Nextflow DSL2 workflow orchestrating raw FASTQ quality control (`FASTQC`), VCF variant filtering, statistics, and annotation (`BCFTOOLS`), cross-tool QC metric aggregation (`MULTIQC`), and PySpark Medallion ingestion into OMOP CDM v5.4 (`MEASUREMENT` table).
