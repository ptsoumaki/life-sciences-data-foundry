...
def configure_s3a_anonymous_access(spark_builder):
...
def read_http_csv(spark: SparkSession, url: str, fallback_path: str = None) -> DataFrame:
...
def load_demographics_data(spark: SparkSession, mode: str = "demo", data_dir: str = None) -> DataFrame:
...
def load_diagnoses_data(spark: SparkSession, mode: str = "demo", data_dir: str = None) -> DataFrame:
    """Ingests clinical diagnoses in either demo or remote open dataset mode."""
    file_path = os.path.join(data_dir, "clinical_diagnoses.csv")
    if mode.lower() == "remote":
...
def load_labs_data(spark: SparkSession, mode: str = "demo", data_dir: str = None) -> DataFrame:
...
def parse_vcf_to_dataframe(spark: SparkSession, vcf_path: str, max_rows: int = None) -> DataFrame:
...
def load_genomics_data(spark: SparkSession, mode: str = "demo", data_dir: str = None) -> DataFrame:
    """Ingests VCF genomic variant calls in either demo or remote open dataset mode."""
    vcf_file = os.path.join(data_dir, "genomic_variants.vcf")
    if mode.lower() == "remote":
